...
def main():
    ...
    ckpt_paths = {
        "MNIST":   "checkpoints/LeNet_MNIST.pth",
        "cifar10": "checkpoints/LeNetCIFAR10.pth",
        "cifar100":"checkpoints/LeNetCIFAR100.pth",
    }
    ...
    if dataset == 'MNIST':
        ...
        channel = 1
        hidden = 588
        dst = datasets.MNIST(data_path, train=True, download=True, transform=tt)
        ...
        channel = 3
        hidden = 768
        dst = datasets.CIFAR100(data_path, train=True, download=True, transform=tt)
        ...
        channel = 3
        hidden = 768
        dst = datasets.CIFAR10(data_path, train=True, download=True, transform=tt)
    ...
    for idx_net in range(num_exp):
        ...
        if dataset == 'MNIST':
            ...
            net = LeNetCIFAR100(channel=channel, num_classes=num_classes)
        else:
            raise ValueError("unknown dataset")

        # --- load pretrained weights ---
        ckpt = ckpt_paths[dataset]
        ...
        for method in ['iDLG', 'iDLG_masked']:
            ...
            for iters in range(Iteration):
                ...
                def closure():
                    optimizer.zero_grad()
                    pred = net(norm_for_net(dummy_data))
                    ...
                    if method == 'iDLG':
                        ...
                        keep_param_indices = {0,1,2,3,L-2,L-1}

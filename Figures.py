...
def save_recon_panel(num_exp, panel_gt_pil, panel_idlg_pil, panel_masked_pil, save_dir, block_idx, dataset, mask_desc: str, timestamp_str: str):
    """
    Save a 3xN panel:
    ...
    for j in range(n):
        ...
        ax.set_title(f"exp {block_idx * num_exp + j}")
        ax.axis('off')
    ...
    safe_mask = ''.join(ch if (ch.isalnum() or ch in ['-', '_']) else '_' for ch in mask_desc)
    out_path = os.path.join(save_dir, f"{num_exp}_exp_{safe_mask}_{timestamp_str}_block_{block_idx:03d}.png")
